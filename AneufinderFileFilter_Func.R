...
###------loop over all samples to generate QC data and filters------###
	...
	message("- Too high copy numbers: ", filter.copy.number)
    ...
    #create empty datacollection list
		...
		for(sample in sampleIDs) {
		    ...
		    writeLines(c("=== Selected model ===", c(paste0("- ", selected.model)), "",
                     ...
                     c(paste0("- Maximum weighted copy number = ", filter.copy.number, ", ", max.filter.copy.number)), "", 
		    ...
		    #load Files and models based on selected model
			  ...
			  selected.files.min.bhat <- rownames(myQC[(myQC$selected == TRUE & myQC$bhattacharyya >= min.bhattacharyya),])
			  selected.files.NA.bhat <- rownames(myQC[(myQC$selected == TRUE & is.na(myQC$bhattacharyya)),])
		    selected.files <- append(selected.files.min.bhat, selected.files.NA.bhat)                     
			  myQC$selected <- rownames(myQC) %in% selected.files 
			  }
			  ...
			  #obtain names of cells that fulfil - weighted copy number - criteria
			  if (filter.copy.number == TRUE){
			    for (my.file in my.files){
			      ...
			      if (models[[my.file]]$qualityInfo$total.read.count != 0){ 
			        ...
			        if (myQC[my.file,'weighted.copy.number'] >= max.filter.copy.number){
          ...
          if (set.plot.heterogeneity == TRUE){
  					message("Plot Anueploidy and Heterogeneity...")
